...
- Diataxis classification output (from the `documentation-diataxis` skill, when run as part of the orchestrated review).
...
5. **Cross-References**:
   ...
   - Prefer native internal cross-reference roles (`:ref:`, `:doc:`, `{ref}`, `{doc}`, `{numref}`) over plain relative page links.
   - Flag unresolved or brittle cross-references, not valid uses of `:doc:`/`{doc}`.
